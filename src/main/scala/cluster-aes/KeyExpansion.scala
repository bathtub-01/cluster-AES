...
class TmpStruct extends Bundle {
  val keys = Vec(32, UInt(8.W))
  val keylength = UInt(2.W)
  val rounds = UInt(4.W)
}

class KSubBytes extends Module {
  val pipeline_layer = 2
  val io = IO(new Bundle {
    val para_in = Input(new KSBPara)
    val para_out = Output(new KSBPara)
  })

  val sboxs = Seq.fill(4)(Module(new AESSBox))
  for (i <- 0 until 4) {
      sboxs(i).io.in := io.para_in.word(i)
      io.para_out.word(i) := sboxs(i).io.out
  }
...
class KeyExpansion extends Module {
  ...
  // 1. getting user_key from input
  // 2. generating roundkeys
  // 3. offer roundkeys from read port
  ...
  // val Buffer = RegNext(KSubBytesModule.io.para_out, 0.U)
  // val Tmp = Reg(Vec(4, new TmpStruct))
  ...
  KSubBytesModule.io.para_in.word := TaskSelect(TaskCounter) // they should be wire and given explicitly!
  
  def takeWord(in: Vec[UInt], nth: Int): Vec[UInt] = VecInit(in.slice(nth * 4, nth * 4 + 4)) // start with 0
  ...
  def chooseRcon192(round: UInt): UInt = RoundConst(Rcon.m192(round))
  def chooseRcon256(round: UInt): UInt = RoundConst(round >> 1)

  // io.user_key.ready := IsIdle.reduce(_ || _) // leave taskID management to outside world
  io.user_key.ready := (TaskCounter + 1.U === io.taskID) && IsIdle(io.taskID)
  when(io.user_key.fire) { // 提前一周期交火
  ...
  for (i <- 0 until 4) {
    when(TaskCounter + 1.U === i.U && !IsIdle(i)) { // there is a one-clk delay!
    ...
    switch(KeyLength(i)) {
      is(0.U) { // 128-bit
        when(TaskIter(i) === 0.U) {
          ...
          io.complete_taskID.bits := i.U // reset needed?
        }
      }
      is(1.U) { // 192-bit
        TaskSelect(i) := DontCare
      }
      is(2.U) { // 256-bit
        TaskSelect(i) := DontCare
      }
