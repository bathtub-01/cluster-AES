...
class ControllerTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  def setWork(id: Int, length: Int, source_addr: BigInt, dest_addr: BigInt)(implicit dut: Controller) = {
    require(id < 16)
  ...
  "Controller" should "pass" in {
    test(new Controller(3)).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      ...
      setWork(1, 1, 0x1000_0000, 0x1100_0000)
      feedText(Seq(0x03020100.U, 0x07060504.U, 0x0b0a0908.U, 0x0f0e0d0c.U))
      setKey(9, Seq(0x01.U, 0x01.U, 0x01.U, 0x01.U, 
                    0x01.U, 0x01.U, 0x01.U, 0x01.U, 
                    0x01.U, 0x01.U, 0x01.U, 0x01.U, 
                    0x01.U, 0x01.U, 0x01.U, 0x01.U))
      setWork(9, 1, 0x0900_0000, 0x0990_0000)
      feedText(Seq(0x01010101.U, 0x01010101.U, 0x01010101.U, 0x01010101.U))
